...
/**
 * ベータ分布モデルによる検定に基づく, 連続分布乱数発生器のテスティングフレームワーク. <br>
 * サンプルサイズ(9999)による経験累積分布を用いて乱数のテストを行う. <br>
 * 9999個の乱数を生成し, 1000,2000,...,9000番目の値に対応する累積確率の解析値と順位統計量が満たす確率分布を比較して,
 * 逸脱があるかを確かめる. <br>
 * <br>
 * 9999個の確率変数の第(1000k)位統計量に対する累積確率の値はBeta(1000k,10000-1000k)に従う. <br>
 * したがって, 乱数から得られた第(1000k)位統計量を, 本来従うべき分布の累積分布関数を用いて累積確率に換算し,
 * その値がBeta(1000k,10000-1000k)とどれほど逸脱しているかを見ることで, 乱数の妥当性を判定できることになる. <br>
 * <br>
