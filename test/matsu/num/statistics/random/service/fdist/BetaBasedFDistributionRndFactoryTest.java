...
public class BetaBasedFDistributionRndFactoryTest {
    ...
    private static final FDistributionRnd.Factory FACTORY =
            new BetaBasedFDistributionRndFactory(RandomGeneratorFactoryProvider.byDefaultLib());

    public static class ファクトリの境界値テスト {

        @Test(expected = None.class)
        public void test_境界内最小値() {
            assertThat(FACTORY.acceptsParameters(LOWER_LIMIT_DEGREES_OF_FREEDOM, LOWER_LIMIT_DEGREES_OF_FREEDOM), is(true));
            FACTORY.instanceOf(LOWER_LIMIT_DEGREES_OF_FREEDOM, LOWER_LIMIT_DEGREES_OF_FREEDOM);
        }

        @Test(expected = None.class)
        public void test_境界内最大値() {
            assertThat(FACTORY.acceptsParameters(UPPER_LIMIT_DEGREES_OF_FREEDOM, UPPER_LIMIT_DEGREES_OF_FREEDOM), is(true));
        ...
        public void test_境界最小値外() {
            assertThat(
                    FACTORY.acceptsParameters(Math.nextDown(LOWER_LIMIT_DEGREES_OF_FREEDOM), LOWER_LIMIT_DEGREES_OF_FREEDOM),
                    is(false));
            assertThat(
                    FACTORY.acceptsParameters(LOWER_LIMIT_DEGREES_OF_FREEDOM, Math.nextDown(LOWER_LIMIT_DEGREES_OF_FREEDOM)),
                    is(false));
        }
        ...
        public void test_境界最大値外() {
            assertThat(
                    FACTORY.acceptsParameters(Math.nextUp(UPPER_LIMIT_DEGREES_OF_FREEDOM), UPPER_LIMIT_DEGREES_OF_FREEDOM),
                    is(false));
            assertThat(
                    FACTORY.acceptsParameters(UPPER_LIMIT_DEGREES_OF_FREEDOM, Math.nextUp(UPPER_LIMIT_DEGREES_OF_FREEDOM)),
                    is(false));
        }
    }
